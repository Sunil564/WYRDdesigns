...
**Fragment shader.** Soft circular falloff, squared so the core stays tight, additive blending, `depthWrite` and `depthTest` off. Colour mixes `--color-line` to `--color-muted` by the per point random, and to `--color-signal` for accent points. Accent selection is `step(0.917, aRandom)`, which is 8.3 percent, one in twelve.

**Palette comes from `getComputedStyle`.** The shader reads `--color-line`, `--color-muted`, and `--color-signal` off the root element at mount and passes them as uniforms. No hex value appears outside `globals.css`.
